    ...
    window.__tinyHostApi = {{
        ...
        private static string ResolveTinyMceLanguageCode(string cultureName)
        {
            ...
            var variant = files.FirstOrDefault(f =>
                f.StartsWith(iso2 + "_", StringComparison.OrdinalIgnoreCase));
            if (variant != null)
                return variant;

            // 3) Fallback: variant with dash ("xx-YY") if any
            variant = files.FirstOrDefault(f =>
                f.StartsWith(iso2 + "-", StringComparison.OrdinalIgnoreCase));
            if (variant != null)
                return variant;

            // 4) Nothing usable found -> fall back to English
            return "en";
        }
        ...
        public async Task SetHtmlAsync(string htmlContent)
        {
            ...
            if (!_isEditorReady || _webView?.CoreWebView2 == null)
                return;
