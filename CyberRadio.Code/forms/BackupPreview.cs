...
namespace RadioExt_Helper.forms
{
    public sealed partial class BackupPreview : Form
    {
        ...
        private void BackupPreview_Load(object sender, EventArgs e)
        {
            ...
            lblCompressionLevel.Text = string.Format(GlobalData.Strings.GetString("UsingCompressionLevel") ?? "Using compression level: {0}", _backupManager.BackupCompressionLevel);
        ...
        private async Task StartPreviewLoading()
        {
            ...
            catch (Exception ex)
            {
                ...
                this.SafeInvoke(() =>
                {
                    var text = GlobalData.Strings.GetString("BackupPreviewError") ?? "An error occurred while loading the backup preview.";
        ...
        private void OnBackupPreviewCompleted((List<FilePreview> Previews, long TotalSize, long EstimatedCompressedSize) previewTuple)
        {
