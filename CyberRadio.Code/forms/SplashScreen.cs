    ...
    public partial class SplashScreen : Form
    {
        ...
        private async Task<List<string>> PerformBackgroundTasks()
        {
            ...
            if (config != null)
            {
                ...
                UpdateStatus(GlobalData.Strings.GetString("SplashScreen_MigratedSettings") ?? "Settings migrated successfully.");
                await Task.Delay(600);
                statusMessages.Add("Settings migrated successfully.");
            }
            else
            {
                UpdateStatus(GlobalData.Strings.GetString("SplashScreen_MigratedSettingsNo") ?? "Settings migration not needed.");
                await Task.Delay(600);
