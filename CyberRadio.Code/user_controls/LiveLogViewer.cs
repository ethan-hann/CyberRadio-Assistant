// LiveLogViewer.cs : RadioExt-Helper
...
/// </summary>
/// <remarks>
///     LiveLogViewer enables users to observe log file updates instantly, filter log entries by
...
public partial class LiveLogViewer : UserControl, IUserControl
{
    ...
    /// </summary>
    /// <remarks>
    ///     This constructor sets up the component and subscribes to the Disposed event. Use this
    ///     constructor to create and display a live log viewer in the application.
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    ///     If the specified log file does not exist or the path is invalid, the monitoring
    ///     process will not start. This method loads existing log entries and applies any active filters before
    ///     monitoring begins.
    /// </remarks>
    /// <param name="logFilePath">
    ///     The path to the log file to monitor. If null or whitespace, the default log file path is
    ///     used.
    /// </param>
    ...
    /// </summary>
    /// <remarks>
    ///     After calling this method, the file watcher will no longer raise events for file
    ...
    private void ReadNewEntries()
    {
        ...
        while (reader.ReadLine() is { } line) TryAddEntry(line);
    ...
    private void QueueReadNewEntries()
    {
        lock (_watcherDebounceLock)
        {
            _watcherDebounceTimer ??= new Timer(static state =>
            {
    ...
    private void ApplyFilter()
    {
        ...
        {
            var rowIndex = dgvLogs.Rows.Add(entry.Timestamp?.ToString("yyyy-MM-dd HH:mm:ss.ffff") ?? string.Empty,
    ...
    private void InitializeLayoutParser()
    {
        ...
        _messageTokenIndex = Array.FindIndex(_layoutTokens, token => token.Equals("message", StringComparison.Ordinal));
    }
    ...
    private void copyLineToolStripMenuItem_Click(object sender, EventArgs e)
    {
        ...
        catch (Exception ex)
        {
            ...
            ToastNotification.Show(this, string.Format(Strings.Toast_FailedToCopyLogLine, ex.Message), ToastType.Error,
                3000);
        }
    }
